...
| [mysteriumnetwork/go-openvpn](https://github.com/mysteriumnetwork/go-openvpn) | Go | Reconnecting reasons, full connection lifecycle sequences |
| [NordSecurity/gopenvpn](https://github.com/NordSecurity/gopenvpn) | Go | `>STATE:` with hostname, `>PASSWORD:Auth-Token:` |
...
|-------|-----------|---------------|
| Spec conformance | 194 protocol tests from fixtures captured from real OpenVPN sessions and client libraries | Correct parsing of all notification types, response formats, state names |
| Defensive/injection | 62 tests in `tests/defensive/` | Newline injection, `END` injection, quote breakout, NUL bytes, multi-line truncation, Unicode edge cases |
| Property-based | 35 proptest strategies in `tests/proptest_roundtrip.rs` | Framing invariants (single `\n`), no bare `END`, codec state independence |
